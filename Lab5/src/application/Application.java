...
import abs.Writer.OrbitFileWriter;
import abs.Writer.UseBuffer;
import abs.Writer.UsePrintWriter;
import abs.Writer.UseStream;
...
final class TrackGamePanel extends MyPanel {
    ...
    TrackGamePanel(final JFrame parentFrame) {
    ...
    private void drawTrackGame(final int width, final int height) {
    ...
    public void actionPerformed(final ActionEvent e) {
        if (e.getSource() == select) {
            ...
            try {
                trackGame.addAthlete(PhysicalObjectPool.getInstance().athlete(name, number, country, age, score));
...
//                repaint();
    ...
    private void clickConfirm() {
        if (selectedPath == null) {
            ...
            btnScanner.addActionListener(e -> {
                try {
                    trackGame = (TrackGame) CircularOrbit.parseFile(new TrackGameScannerParser(), selectedPath);
                    remove(btnScanner);
                    remove(btnBuffer);
                    remove(btnReader);
                    newComponents();
            ...
            btnReader.addActionListener(e -> {
                try {
                    trackGame = (TrackGame) CircularOrbit.parseFile(new TrackGameReaderParser(), selectedPath);
                    remove(btnScanner);
                    remove(btnBuffer);
                    remove(btnReader);
                    newComponents();
            ...
            btnBuffer.addActionListener(e -> {
                try {
                    trackGame = (TrackGame) CircularOrbit.parseFile(new TrackGameBufferParser(), selectedPath);
                    remove(btnScanner);
                    remove(btnBuffer);
                    remove(btnReader);
                    newComponents();
    ...
    private void newComponents() {
        ...
        // 写回文件
        JButton btnWriteBack = new JButton("将修改写回文件");
        add(btnWriteBack);
        btnWriteBack.addActionListener(e -> {
            String choice = JOptionPane.showInputDialog("请输入要用什么方式写入文件：\n"
                    + "1. PrintWriter\n2. Stream\n3. Buffered Stream");

            switch (choice) {
                case "1":
                    try {
                        OrbitFileWriter.writeBack(trackGame, new UsePrintWriter());
                        log.info("写入文件成功");
                ...
                case "2":
                    try {
                        OrbitFileWriter.writeBack(trackGame, new UseStream());
                        log.info("写入文件成功");
                    } catch (IOException e1) {
                        e1.printStackTrace();
                        JOptionPane.showMessageDialog(null, "发生未知错误");
                ...
                case "3":
                    try {
                        OrbitFileWriter.writeBack(trackGame, new UseBuffer());
                        log.info("写入文件成功");
                    } catch (IOException e1) {
                        e1.printStackTrace();
                        JOptionPane.showMessageDialog(null, "发生未知错误");
                    ...
                    break;
                default:
                    JOptionPane.showMessageDialog(null, "请输入正确选择");
            }
    ...
    private void fasterLaterArrange() {
        // 强者后出场
...
class AtomStructurePanel extends MyPanel {
    ...
    private Logger log = Logger.getLogger(AtomStructurePanel.class);
...
//                Random random = new Random(System.currentTimeMillis());   // to generate random angle
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == select) {
            ...
            try {
                ...
                atomStructure.addToTrack(PhysicalObjectPool.getInstance().electron(), level, level, System.currentTimeMillis() % 360);
                JOptionPane.showMessageDialog(null, "添加成功", "成功", JOptionPane.PLAIN_MESSAGE);
                repaint();
            } catch (NumberFormatException ex) {
                ex.printStackTrace();
            ...
            } catch (ElectronNotExistException e1) {
    ...
    private void clickConfirm() {
        if (selectedPath == null) {
            ...
            btnScanner.addActionListener(e -> {
                try {
                    atomStructure = (AtomStructure) CircularOrbit.parseFile(new AtomStructureScannerParser(), selectedPath);
                    remove(btnScanner);
                    remove(btnBuffer);
                    remove(btnReader);
                    newComponents();
            ...
            btnReader.addActionListener(e -> {
                try {
                    atomStructure = (AtomStructure) CircularOrbit.parseFile(new AtomStructureReaderParser(), selectedPath);
                    remove(btnScanner);
                    remove(btnBuffer);
                    remove(btnReader);
                    newComponents();
            ...
            btnBuffer.addActionListener(e -> {
                try {
                    atomStructure = (AtomStructure) CircularOrbit.parseFile(new AtomStructureBufferParser(), selectedPath);
                    remove(btnScanner);
                    remove(btnBuffer);
                    remove(btnReader);
                    newComponents();
    ...
    private void newComponents() {
        ...
        btnWriteBack.addActionListener(e -> {
            ...
            switch (choice) {
                case "1":
                    try {
                        OrbitFileWriter.writeBack(atomStructure, new UsePrintWriter());
                        log.info("写入文件成功");
                    } catch (IOException e1) {
                        e1.printStackTrace();
                        JOptionPane.showMessageDialog(null, "发生未知错误");
                ...
                case "2":
                    try {
                        OrbitFileWriter.writeBack(atomStructure, new UseStream());
                        log.info("写入文件成功");
                    } catch (IOException e1) {
                        e1.printStackTrace();
                        JOptionPane.showMessageDialog(null, "发生未知错误");
                ...
                case "3":
                    try {
                        OrbitFileWriter.writeBack(atomStructure, new UseBuffer());
                        log.info("写入文件成功");
                    } catch (IOException e1) {
                        e1.printStackTrace();
                        JOptionPane.showMessageDialog(null, "发生未知错误");
...
class SocialNetworkPanel extends MyPanel {
    ...
    private Logger log = Logger.getLogger(SocialNetworkPanel.class);
...
//    private Random random = new Random(System.currentTimeMillis());
    ...
    SocialNetworkPanel(JFrame parent) {
        this.parent = parent;
    ...
    private void drawSocialNetworkCircle(int width, int height) {
...
//            double physical = circle.getPhysicalDistance(inputs[0], inputs[1]);
    ...
    private void clickConfirm() {
        if (selectedPath == null) {
            ...
            btnScanner.addActionListener(e -> {
                try {
                    circle = (SocialNetworkCircle) CircularOrbit.parseFile(new SocialNetworkCircleScannerParser(), selectedPath);
                    newComponents();
                    remove(btnScanner);
                    remove(btnBuffer);
                    remove(btnReader);
            ...
            btnBuffer.addActionListener(e -> {
                try {
                    circle = (SocialNetworkCircle) CircularOrbit.parseFile(new SocialNetworkCircleBufferParser(), selectedPath);
                    newComponents();
                    remove(btnScanner);
                    remove(btnBuffer);
                    remove(btnReader);
            ...
            btnReader.addActionListener(e -> {
                try {
                    circle = (SocialNetworkCircle) CircularOrbit.parseFile(new SocialNetworkCircleReaderParser(), selectedPath);
                    remove(btnScanner);
                    remove(btnBuffer);
                    remove(btnReader);
                } catch (FileNotFoundException e1) {
    ...
    private void newComponents() {
        ...
        btnWriteBack.addActionListener(e -> {
            ...
            switch (choice) {
                case "1":
                    try {
                        OrbitFileWriter.writeBack(circle, new UsePrintWriter());
                        log.info("写入文件成功");
                    } catch (IOException e1) {
                        e1.printStackTrace();
                        JOptionPane.showMessageDialog(null, "发生未知错误");
                ...
                case "2":
                    try {
                        OrbitFileWriter.writeBack(circle, new UseStream());
                        log.info("写入文件成功");
                    } catch (IOException e1) {
                        e1.printStackTrace();
                        JOptionPane.showMessageDialog(null, "发生未知错误");
                ...
                case "3":
                    try {
                        OrbitFileWriter.writeBack(circle, new UseBuffer());
                        log.info("写入文件成功");
                    } catch (IOException e1) {
                        e1.printStackTrace();
                        JOptionPane.showMessageDialog(null, "发生未知错误");

...
public class TrackGame implements OrbitAble {
     ...
     */
    public boolean addAthlete(Athlete athlete, int track, int angle) {
        ...
        if (nameMap.keySet().contains(athlete.getName())) {
            ...
            assert orbit.addToTrack(athlete, track, track, angle);
...
//        Athlete old = nameMap.put(athlete.getName(), athlete);
//
//        if (old != null) {
//            // contains an athlete with same name
//            // undo changes
//            nameMap.put(athlete.getName(), old);
...
//        }
     ...
     */
    private void autoAddTrack() {
        for (int i = 0; i < numberOfTracks; i++) {
            orbit.addTrack(i + 1, i + 1);
        }
    }
    ...
    public boolean removeAthlete(Athlete athlete) {
        boolean b = athletes.remove(athlete);
        if (!b) return false;
    ...
    public boolean removeAthlete(String name) {
        ...
        if (athlete == null) return false;      // does not exist
        // the athlete with passed in name must exist
        return removeAthlete(athlete);
    }
     ...
     * @return
     */
    public boolean addTrack(int level, double radius) {
    ...
    public boolean removeTrack(int level) {
        if (!orbit.containsLevel(level)) return false;  // level does not exist
